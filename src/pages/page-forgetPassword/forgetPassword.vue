      ...
      <div>
        <template v-if="step1">
          <ul class="forgetPasswordFrom">
            <li><span>请输入手机号码</span> <input type="tel" maxlength="11" @blur="checkTel" v-model="tel" class="userInput"></li>
            <p class="info">{{telInfo}}</p>
            <li>
              <span>请输入验证码</span><input type="text" @blur="checkCode" v-model="authenticode"  class="userInput">
            ...
            <li><span>请输入密码</span><input type="password" @blur="checkPassword" v-model="password"  class="userInput"></li>
            <p class="info">{{ errText }}</p>
            <li><span>请再次输入密码</span><input type="password" @blur="secondCheck" v-model="confirmpassword"  class="userInput"></li>
            <p class="info">{{secondText}}</p>
            <li><button type="button" class="baseBtn orangeGrad btnNormal"
                        v-if="step1" @click="checkedCode">下一步</button></li>
          </ul>
        </template>
