<template>
  <div class="manage">
    ...
    <ul class="manageContent" v-if="managesList!=''">
      <li v-for="manage in managesList" class="manageLi" :key="manage.id">
        <div class="manage-title">{{ manage.productName }}</div>
        <div class="manage-main">
          <p><span>还款金额:</span>&nbsp;{{ manage.amount | FilterAmount}} <span style="color: #f0b954;">({{manage.period | Num}}/{{ manage.total | Num}}期)</span></p>
          <p><span>还款日期:</span>&nbsp;{{ manage.repayDate | Time}}</p>
          <button type="button" :disabled="payed({hide: manage.hide,status: manage.status})" class="manage-btn btnNormal"
...
//        console.log('kk',hide,status)
      ...
      showPropPay: function (manage) {
        console.log(`hide:${manage.hide},status:${manage.status}`)
        if( manage.hide === '1' && manage.status === '2'){
