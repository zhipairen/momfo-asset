<template>
  <div class="madeProject">
    <div class="madeProject-basic">
      ...
      <p>已发行金额: &nbsp;{{issueInfo.ptotal | FilterAmount}}万元&nbsp;({{issueInfo.pers}}%)</p>
    </div>
    <ul class="madeProject-content">
      <li v-for="issue in issueList" class="manageLi" @click="seeProductDetails({productId:issue.pid, productName: issue.productName})">
        ...
        <div class="manage-main">
          ...
          <p><span>资金方:</span>&nbsp;{{ issue.financingName }}</p>
          <p><span>待还金额:</span>&nbsp;{{ issue.sumAmount | FilterAmount}}</p>
        </div>
      </li>
    </ul>
